...
public class AuthController {
	...
	public static PSH inscription(String nom, String prenom, String email, String motDePasse, String numeroEtudiant,
			String typeHandicap) {
		...
		try {
			...
			// Construire et retourner l'objet PSH (non connecté — il doit attendre
			// validation)
	...
	private static boolean emailExiste(Connection conn, String email) {
		try (PreparedStatement stmt = conn.prepareStatement("SELECT 1 FROM utilisateur WHERE email = ?")) {
			stmt.setString(1, email);
			try (ResultSet rs = stmt.executeQuery()) {
				return rs.next();
			}
	...
	private static PSH chargerPSH(Connection conn, int idUser, String nom, String prenom, String email,
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (!rs.next())
					throw new SQLException("PSH introuvable pour id=" + idUser);
				PSH psh = new PSH(nom, prenom, email, motDePasse, rs.getString("typeHandicap"),
	...
	private static Administrateur chargerAdmin(Connection conn, int idUser, String nom, String prenom, String email,
			String motDePasse) throws SQLException {
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (!rs.next())
					throw new SQLException("Admin introuvable pour id=" + idUser);
				Administrateur admin = new Administrateur(nom, prenom, email, motDePasse, rs.getString("matricule"));
				admin.setId(idUser);
