...
public class DossierController {
	...
	public static int insertDossier(Dossier dossier) {
		...
		String sql = "INSERT INTO dossier (description, statut, typeDossier, archive, idAuteur, dateCreation, dateMaj) "
				+ "VALUES (?, ?, ?, ?, ?, ?, ?)";
	...
	public static List<Demande> getAllDemandes() {
		...
		String sql = "SELECT d.idDossier, d.description, d.dateCreation, d.dateMaj, "
				+ "d.statut, d.archive, d.dateArchivage, d.idAuteur, " + "dm.typeDemande, dm.commentaire "
		...
		try (PreparedStatement stmt = conn.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
			while (rs.next())
				liste.add(hydrateDemande(rs, rs.getInt("idAuteur")));
		} catch (SQLException e) {
	...
	// N derniers dossiers d'un auteur (demandes + réclamations mélangés) —
	...
	public static List<Dossier> getDerniersParAuteur(int idAuteur, int limite) {
		...
		String sqlDm = "SELECT d.idDossier, d.description, d.dateCreation, d.dateMaj, "
				+ "d.statut, d.archive, d.dateArchivage, " + "dm.typeDemande, dm.commentaire "
				+ "FROM dossier d JOIN demande dm ON d.idDossier = dm.idDossier "
				+ "WHERE d.idAuteur = ? ORDER BY d.dateMaj DESC LIMIT ?";

		String sqlRcl = "SELECT d.idDossier, d.description, d.dateCreation, d.dateMaj, "
				+ "d.statut, d.archive, d.dateArchivage, " + "r.motif, r.reponse, r.idDemande "
		...
		try (PreparedStatement s1 = conn.prepareStatement(sqlDm)) {
			...
			try (ResultSet rs = s1.executeQuery()) {
				while (rs.next())
					liste.add(hydrateDemande(rs, idAuteur));
			}
		} catch (SQLException e) {
		...
		try (PreparedStatement s2 = conn.prepareStatement(sqlRcl)) {
			...
			try (ResultSet rs = s2.executeQuery()) {
				while (rs.next())
					liste.add(hydrateReclamation(rs, idAuteur));
			}
		} catch (SQLException e) {
	...
	public static boolean archiverDossier(Dossier dossier) {
		...
		String sql = "UPDATE dossier SET archive = TRUE, statut = 'CLOTUREE', dateArchivage = ?, dateMaj = ? "
				+ "WHERE idDossier = ?";
	...
	public static boolean appartientA(int idDossier, int idUtilisateur) {
		...
		try (PreparedStatement stmt = conn.prepareStatement("SELECT idAuteur FROM dossier WHERE idDossier = ?")) {
			stmt.setInt(1, idDossier);
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next())
					return rs.getInt("idAuteur") == idUtilisateur;
			}
		} catch (SQLException e) {
	...
	private static Demande hydrateDemande(ResultSet rs, int idAuteur) throws SQLException {
		...
		Demande d = new Demande(rs.getString("description"), proxy, TypeDemande.valueOf(rs.getString("typeDemande")));
	...
	private static Reclamation hydrateReclamation(ResultSet rs, int idAuteur) throws SQLException {
		Utilisateur proxy = buildProxy(idAuteur);
		Reclamation r = new Reclamation(rs.getString("description"), proxy, rs.getString("motif"));
